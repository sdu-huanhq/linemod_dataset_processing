...
def project(xyz, K, RT):
    ...
    RT: [3, 4]
    """
    #! Here xyz is the row vector, but in the formula, it is the column vector, so need a transpose
    xyz = np.dot(xyz, RT[:, :3].T) + RT[:, 3:].T
    xyz = np.dot(xyz, K.T)
    xy = xyz[:, :2] / xyz[:, 2:] #! Z[x,y,1] = [X,Y,Z]
    return xy
...
class OpenGLRenderer(object):
    intrinsic_matrix = {
        'linemod': np.array([[572.4114, 0., 325.2611],
                              ...
                              [0., 0., 1.]]), #! linemod dataset camera intrinsic
    }
    models = {}
    ...
    def load_ply(self, ply_path):
        ...
        # Read header
        while True:
            #! read each line in ply file, delete the newline character and return character
            #! rstrip() can remove the specified character from the end of a string
            
            line = f.readline().rstrip('\n').rstrip('\r') # Strip the newline character(s)
            if line.startswith('element vertex'): 
                
                #! Splits a string by the specified symbol, defaults to whitespace, and converts the split string into a list
                n_pts = int(line.split()[-1]) #! read the number of points
                header_vertex_section = True
                header_face_section = False
            elif line.startswith('element face'):
                n_faces = int(line.split()[-1]) #! read the number of faces
                header_vertex_section = False
